...
class _ProfileAvatarWidgetState extends State<ProfileAvatarWidget> {
  ...
  Future<UserModel> readingUserJsonData() async {
    ...
    if (response.statusCode == 200) {
      readUserJsonFileContent = userModelListFromJson(response.body)
          .where((element) => element.personalEmail == widget.email || element.workEmail == widget.email)
          .toList()[0];
  ...
  void _upload(File imageFile) {
    ...
    /// string to uri
    var uri = Uri.parse("${AppUrl.addAndUpdateProfileImage}${UserProfile.personalEmail}");
